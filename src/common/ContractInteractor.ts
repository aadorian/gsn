...
import { Address, EventName, IntString, Web3ProviderBaseInterface } from './types/Aliases'
...
import { ActiveManagerEvents, RelayWorkersAdded } from './types/GNSContractsEvents'
...
export default class ContractInteractor {
  ...
  async _resolveDeployment (): Promise<void> {
    ...
    if (this.deployment.paymasterAddress != null) {
      ...
      await this._resolveDeploymentFromRelayHub()
    } else {
  ...
  async _resolveDeploymentFromRelayHub (): Promise<void> {
  ...
  async _createRelayHub (): Promise<IRelayHubInstance> {
    if (this.deployment.relayHubAddress == null) {
      throw new Error('RelayHub address is not initialized!')
    }
    return await this.IRelayHubContract.at(this.deployment.relayHubAddress)
  }
